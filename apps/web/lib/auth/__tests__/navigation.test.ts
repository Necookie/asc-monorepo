...
describe('Public navigation account data', () => {
  it('serializes only the account fields needed by navigation', () => {
    ...
    expect(getNavigationAccount({ status: 'RESOLVED', member })).toEqual({ status: 'MEMBER', username: 'alex', displayName: 'Alex', avatar: null, slug: 'alex', isAdmin: false, isModerator: false, isOwner: false });
  });
